...
namespace WatermarkScreen
{
    public partial class Watermark : Form
    {
        ...
        public string Hour = DateTime.Now.ToLongTimeString();
        ...
        {
            Text = "HS PREVENT \r\n" + "Direitos reservados \r\n" + Environment.UserName + "\r\n" + DateTime.Now.ToLongTimeString() + " " + DateTime.Now.ToString("dd/MM/yyyy");
            SizeF szF1 = e.Graphics.MeasureString(Text, this.Font);
            

            /// "SizeF" Stores an ordered pair of floating-point numbers, typically the width and height of a rectangle, 
            /// "szF1" receive de value of return of "MeasureString()" and initializes a new instance of the SizeF structure from the specified dimensions. 
            /// "MeasureString()" Measures the specified string when drawn with the specified Font text.
            Console.WriteLine(szF1.Width);
            Console.WriteLine(szF1.Height);
            int max = Math.Max(this.Width, this.Height);

            //object used to draw lines
            ...
            for (int i = 0; i < hei2; i++)
            {
                if ((i % 2) == 0)
                {
                    ...
                    {
                        e.Graphics.DrawString(Text, this.Font, test, (float)(wid1 + szF1.Width * j), (float)(hei1 + szF1.Height * i));
                ...
                else
                {
                    ...
                    {
                        e.Graphics.DrawString(Text, this.Font, test, (float)(wid1 + szF1.Width * (j + 1)), (float)(hei1 + szF1.Height * i));

                    }
                    // for (double x = (1 * (int)szF1.Width) ; x <= this.Width; x = x + (2 * (int)szF1.Width))
                    //{

                    // e.Graphics.DrawString(Text, this.Font, test, (float)x, (float)y);
                    //Bibliotecas System.Drawing tem a classe Graphics e Drawing2D
                    //DrawString, DrawPath, Pen, SolidBrush,Brush
                    //}
                }
            }
            e.Graphics.DrawString(Environment.MachineName, this.Font, test, this.Width - 250, this.Height - 100);
            this.Update();
            this.Invalidate();
            Thread.Sleep(3000);
        }
